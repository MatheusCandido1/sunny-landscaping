    ...
    function index(Request $request)
    {
      if($request->filter_status == 1)
      {
         $data = DB::table('visits')
            ...
            ->where('visits.has_services','=', $request->filter_status)
            ->get();
     ...
     else if ($request->filter_status == 0 || $request->filter_status == 2){
...
->make(true);
        ...
        ->selectRaw('services.status,CONCAT("#",services.id) as service_id,customers.id as customer_id, visits.id as visit_id, customers.name as customer_name, visits.date as visit_date, MONTHNAME(visits.date) as month, services.total')
